...
class MovieModelTestCase(TestCase):
    ...
    def test_movie_update_value(self):
        # Проверка обновления записи
        self.print_info('Start test_movie_update_value')
        # Movie.objects.filter(id=1).update(name="111")
        movie = Movie.objects.get(id=1)
        movie.name = '111'
        movie.save()
        self.assertEqual(movie.name, '111')
        self.print_info('Finish test_movie_update_value')
